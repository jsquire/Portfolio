...
@"
  ...
  public class PhantomSentinal
  {
    public void Start(string filename, string arguments, string workingDirectory, Action<string> writeAction)
    {
...
{  
  if (-not $_.FullyQualifiedErrorId.StartsWith("TYPE_ALREADY_EXISTS"))
  {
    throw
  }
}
...
$sentinal = New-Object PhantomSentinal
$sentinal.Start($PhantomPath, "$($TestScriptPath) $($suiteUri)", $TestScriptContainer, { param($message) WriteToHost $message })